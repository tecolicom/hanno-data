...
# 前世代と比較した結果、述べるに値する変更が見つからなかったときに出す文言。
#
# **比較を実施したときにだけ使うこと。** LLM が使えない・前世代に比較材料が無い・
# API が失敗した、といった「そもそも見ていない」場合にこれを出すと、調べていない
# のに調べたと述べることになる。呼出側は (行, 比較したか) の 2 値で受け取り、
# 比較したときだけこの文言に落とす。
#
# 「軽微な修正」等と断定しないのは、比較が「前回の*要約* × 今回の*本文*」という
# 非対称なもので、原理的に見落としがありうるため。認められなかった、という
# 観測の事実だけを述べる。
#
# 差分行を作るクローラが複数あるので _lib に置く (drop_unchanged_claims と同じ理由)。
DIFF_NO_CHANGE_NOTE = "大きな変更は認められませんでした"

...
- 飯能市では「飲料缶の日」は文字通り飲料缶(アルミ缶・スチール缶)のみ回収する
- 缶詰の缶(食品缶)・スプレー缶は「危険物」(`hazardous`)に分類される
...
### `hazardous`(危険物)— ⚠️ `harmful` ではない

- **`hazardous`** = 危険物 = 物理的危険(発火・爆発・刃物等)
  例: ライター、スプレー缶、電池、缶詰の缶、刃物
- **`harmful`** = 有害ごみ = 化学的・毒性的に有害なもの
  例: 蛍光灯、水銀体温計、農薬

英語ではこの区別があり、東京 23 区など複数の自治体ガイドが `Hazardous` を使う。
飯能市は前者(危険物)のカテゴリのみ存在するため `hazardous` を採用。

将来、有害ごみ(蛍光灯・水銀等)が別カテゴリで必要になった場合は `harmful` を
追加する(現時点では未使用)。
